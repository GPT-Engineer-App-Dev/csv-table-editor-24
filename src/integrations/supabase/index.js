...
export const useUpdateProduct = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ id, ...updates }) => fromSupabase(supabase.from('products').update(updates).eq('id', id)),
        onSuccess: () => {
            queryClient.invalidateQueries('products');
        },
    });
...
export const useUpdateShoppingCartItem = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ id, ...updates }) => fromSupabase(supabase.from('shopping_cart').update(updates).eq('id', id)),
        onSuccess: () => {
            queryClient.invalidateQueries('shopping_cart');
        },
    });
